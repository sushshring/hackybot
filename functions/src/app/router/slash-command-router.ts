...
export class SlashCommandRouter extends ParentRouter {
  protected routes(): void {
    ...
    this.router.post("*", (req, res, next) =>
      this.handleSlashRoute(req, res, next)
    );
  }
  ...
  private async handleSlashRoute(
    req: Request,
    res: Response,
    next: NextFunction
  ) {
    ...
    if (req.body.payload) {
      ...
      try {
        res
          .status(200)
          .send({ text: "Handling...", response_type: "ephemeral" });
        return this.respondQueryHandler.handle(
          RespondQueryCommandHandler.parseSubCommand(requestArgs)
        );
      } catch (error) {
        return next(error);
      }
